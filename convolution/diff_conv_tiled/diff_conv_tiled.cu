...
// function to perform convolution on gpu using tiled approach
__global__ void convGPUTiled(float *N, float *M, float *P, int Mask_Width, int Width)
{
    ...
    // threads with index less than O_TILE_WIDTH
    if (tx < O_TILE_WIDTH)
    {
        // calculate the convolution output for the current output index
        float output = 0.0f;
