...
__global__ void conv3dGPU(float *d_A, float *d_B, float *d_C, int height, int width, int channel, int mask_height, int mask_width, int mask_channel, int output_height, int output_width, int num_masks, int stride)
{
    ...
    // Check if the thread is within the bounds of the output matrix and mask index
    if (Row < output_height && Col < output_width && mask_idx < num_masks)
    {
        ...
        for (int i = 0; i < mask_height; i++)
        {
            for (int j = 0; j < mask_width; j++)
            {
                ...
                {
                    output += d_A[(Row * stride + i) * width * channel + (Col * stride + j) * channel + k] * d_B[mask_idx * mask_height * mask_width * mask_channel + i * mask_width * mask_channel + j * mask_channel + k];
                }
            }
        }
...
void conv3dCPU(float *h_A, float *h_B, float *h_C, int height, int width, int channel, int mask_height, int mask_width, int mask_channel, int output_height, int output_width, int num_masks, int stride)
{
    // chw
            ...
            for (int col = 0; col < output_width; col++)
            {
                ...
                for (int i = 0; i < mask_height; i++)
                {
                    for (int j = 0; j < mask_width; j++)
                    {
                        for (int k = 0; k < mask_channel; k++)
                        {
                            sum += h_A[(row * stride + i) * width * channel + (col * stride + j) * channel + k] * h_B[matrix * mask_height * mask_width * mask_channel + i * mask_width * mask_channel + j * mask_channel + k];
                        }
                    }
                }
...
int main()
{
    ...
    float *d_a, *d_b, *d_c;
    // Matrix sizes
    int height = 20, width = 20, channel = 8;
    int mask_width = 5, mask_height = 5, mask_channel = 8, num_masks = 8;
    int stride = 3;

    int output_height = (height - mask_height) / stride + 1;
    ...
    for (int i = 0; i < 1; i++)
    {
        conv3dCPU(h_a, h_b, h_c_cpu, height, width, channel, mask_height, mask_width, mask_channel, output_height, output_width, num_masks, stride);
        conv3dGPU<<<gridDim, blockDim>>>(d_a, d_b, d_c, height, width, channel, mask_height, mask_width, mask_channel, output_height, output_width, num_masks, stride);
