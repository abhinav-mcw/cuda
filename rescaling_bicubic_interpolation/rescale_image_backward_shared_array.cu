...
__global__ void RescaleImageGpuBlock(unsigned char *imageInput, unsigned char *imageOutput, int width, int height, int output_height, int output_width)
{
    ...
    if (row < output_height && col < output_width)
    {
        ...
        // declaring matrix for shared memory
        __shared__ float weighted_sum[16];
        ...
        weighted_sum[n * 4 + m] = value;

        if (weighted_sum[n * 4 + m] < 0)
            weighted_sum[n * 4 + m] = 0;
        else if (weighted_sum[n * 4 + m] > 255)
            weighted_sum[n * 4 + m] = 255;

        __syncthreads();

        // reduction to compute the sum of the values
        for (unsigned int stride = 1; stride <= 8; stride *= 2)
        {
            ...
            if (t < 8 && (t % stride == 0))
            {
                weighted_sum[2 * t] += weighted_sum[2 * t + stride];
            }
        }
        __syncthreads();
        imageOutput[row * output_width + col] = weighted_sum[0];
    }
}
