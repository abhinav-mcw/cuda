...
__global__ void assignWeightsGpu(float *weightsArray, float u)
{
    float t = 0;
    float a = -0.5;
    ...
    // store weights for m-u
    t = m - 1 - u;
    if (abs(t) < 1)
    {
        weightsArray[m] = (a + 2) * pow(abs(t), 3) - (a + 3) * pow(abs(t), 2) + 1;
    }
    else if (abs(t) >= 1 && abs(t) < 2)
    {
        weightsArray[m] = a * pow(abs(t), 3) - 5 * a * pow(abs(t), 2) + 8 * a * abs(t) - 4 * a;
    }
    else if (abs(t) >= 2)
    {
        weightsArray[m] = 0;
...
__global__ void RescaleImageGpu(unsigned char *imageInput, unsigned char *imageOutput, int width, int height, int output_height, int output_width)
{
    ...
    if (row < output_height && col < output_width)
    {
        ...
        // store weights for m and n in 2 arrays
        assignWeightsGpu<<<1, 4>>>(weights_x, u);
        assignWeightsGpu<<<1, 4>>>(weights_y, v);
...
int main(int argc, char **argv)
{
    ...
    assert(cudaMemcpy(outputImageData, ptrOutputImageDataGpu, output_width * output_height, cudaMemcpyDeviceToHost) == cudaSuccess);
    std::cout << " DONE" << std::endl;
